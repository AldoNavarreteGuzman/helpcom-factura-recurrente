...
**Carve-out explícito a la inmutabilidad — reprocesar una `PENDIENTE_UF` (deuda-tecnica.md ítem 8/9):** `PATCH /api/v1/propuestas/{id}/reprocesar-uf` (solo `ADMINISTRADOR`) es la ÚNICA vía admitida para tocar una propuesta ya persistida, y **solo** aplica a propuestas en estado `PENDIENTE_UF` — `PENDIENTE`, `FACTURADA` y `ANULADA` se rechazan siempre (`409 PROPUESTA_NO_REPROCESABLE`). No viola la regla del snapshot inmutable: una `PENDIENTE_UF` nunca llegó a tener un snapshot real (sus columnas de cálculo quedaron en `0`/`null` a propósito, paso 4 de arriba) — reprocesar la **completa**, no la muta. Reconstruye el mismo `EntradaArmadoPropuesta` desde los campos ya guardados de la fila y vuelve a llamar a `ArmadorPropuesta`/`CalculadoraFacturacion` (la misma vía única de cálculo del ciclo y del CSV, nunca reimplementada) — si la UF ahora está disponible, la propuesta pasa a `PENDIENTE` con el snapshot real; si sigue sin estarlo, queda igual en `PENDIENTE_UF`, sin error (es seguro reintentar cuantas veces haga falta). El acuerdo de precio vigente se **re-resuelve** contra la fecha de facturación en el momento del reproceso (no reutiliza el `acuerdo_id` que la fila ya tenía), así que si el acuerdo cambió legítimamente entre el intento original y el reproceso, el snapshot final refleja la realidad actual. No inserta una fila nueva (actualiza en sitio), así que el índice único parcial del ciclo (`uq_prop_ciclo_periodo`) nunca entra en juego, y funciona igual para propuestas `origen=CICLO` u `origen=CSV`.
